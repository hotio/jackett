...
if [[ ${1} == "screenshot" ]]; then
    SERVICE_IP="http://$(ping -c1 -4 service | sed -nE 's/^PING[^(]+\(([^)]+)\).*/\1/p'):9117"
    NETWORK_IDLE="2"
...
EOF
    ...
    SERVICE="http://service:9117"
    currenttime=$(date +%s); maxtime=$((currenttime+60)); while (! curl -fsSL -b /dev/shm/cookie ${SERVICE} > /dev/null) && [[ "$currenttime" -lt "$maxtime" ]]; do sleep 1; currenttime=$(date +%s); done
    curl -fsSL -b /dev/shm/cookie ${SERVICE} > /dev/null
    ...
    [[ -z ${version} ]] && exit 1
    sed -i "s/{JACKETT_VERSION=[^}]*}/{JACKETT_VERSION=${version}}/g" .drone.yml
